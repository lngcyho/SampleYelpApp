...
public class FetchRestaurantDetailsCallable implements Callable<Response> {
    private final String restaurantId;

    public FetchRestaurantDetailsCallable(String restaurantId) {
    ...
    public Response call() {
        ...
        Request request = constructRequest(GlobalConfig.YELP_FETCH_BUSINESS_DETAILS_URL, this.restaurantId);

        Response response = null;
        try {
            response = client.newCall(request).execute();
            if (!response.isSuccessful()) {
                throw new IOException("FetchRestaurantDetailsCallable onFailure Unexpected code " + response);
            }
        ...
        Log.d(getClass().getName(), "FetchRestaurantDetailsCallable onSuccess");
        return response;
    }
